...
<%
	...
	try {
		sql = "select count(bm_num) from board_main where bm_num = ? and m_id = ?"; //count : 행이 있는지 없는지. 1이면 true 0이면 false. 
		pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, Integer.parseInt(bm_num)); // ? 부분에 넣을 값
		...
		if (cnt == 1) {
			...
			out.print(bm_contents);
			/*sql = "update board_main set bm_subject = ?, bm_contents = ? where bm_num = ?";
			pstmt = conn.prepareStatement(sql);
