...
<title>Ex0825_7</title>
<script type="text/javascript">
	function checkForm() {
/* 		if (document.frm.name.value == "") {
			alert("Please input name.");
			document.frm.name.select(); // 이 위치에 그대로 있도록 설정
		}
		alert(document.frm.name.value); */
		var regExp = /Java/i;
		var str = document.frm.title.value;
		var result = regExp.exec(str); //str과 regExp가 매칭되면 부분문자열 반환
		alert(result[0]);
	}
	...
	function checkLogin() {
		...
		if(document.loginForm.id.value.length < 4 || document.loginForm.id.value.length > 12) {
			alert("Put 4~12 words in ID form.")
			document.loginForm.id.select();
			return;
		}
		
/*  		for(int i = 0; i < document.loginForm.id.value.length; i++) {
			var ch = document.loginForm.id.value.charAt(i);
			
			if((ch < 'a' || ch > 'z') && (ch > 'A' || ch < 'Z') && (ch > '0' || ch < '9')) {
				alert("ID must be lower case.");
				document.loginForm.id.select();
				return;
			}
		}  */
		...
		if(document.loginForm.password.value.length < 4) {
			alert("Put more than 4 words in password form.");
			return;
		}
		if(isNaN(document.loginForm.password.value)) {
			alert("Password is only number.");
			document.loginForm.password.select();
			return;
		}
	...
	<div id="content">
		<section id="main_section">
			<article class="main_article">
				<h1>Main Article</h1>
				<form name="loginForm" action="Ex0825_8.jsp" method="post">
					<p> ID : <input type="text" name="id"></p>
					<p> Password : <input type="password" name="password"></p>
					<p><input type="button" value="send" onclick="checkLogin();"></p>
				</form>
			</article>
			<article class="main_article">
				<h1>Main Article</h1>
				<form name="Member" method="post" action="./Ex0825_8.jsp">
