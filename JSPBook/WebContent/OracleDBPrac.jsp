...
<%
	...
	try {
		...
		out.print("Drive Load Success<br>");
		conn = DriverManager.getConnection(url, user, password);
		out.print("Connection Success<br>");
		sql = "select * from tbltest"; // sql문 작성
		pstmt = conn.prepareStatement(sql); // 미리 컴파일
		rs = pstmt.executeQuery(); // 컴파일 한 객체를 실행
		while(rs.next()) { // 데이터가 있는가? true
			String username = rs.getString("username");
			int age = rs.getInt("age");
			out.print(username + " : " + age + "<br>");
		}
		out.print("Query OK<br>");
	} catch(Exception e) {
