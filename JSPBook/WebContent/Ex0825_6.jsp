...
<title>Ex0825_6</title>
</head>
<body>
<%
	request.setCharacterEncoding("UTF-8");
	String fileUploadPath = "D:\\WebProg\\upload";

	DiskFileUpload upload = new DiskFileUpload(); //파일업로드 객체생성
	
	List items = upload.parseRequest(request); // 요청 파라미터를 전달받음(여러개)
	
	Iterator params = items.iterator(); //반복자를 통해 내부의 다중화 되어있는 데이터를 하나하나 뽑아냄
	...
	while(params.hasNext()) {
		...
		if(!fileItem.isFormField()) { //요청 파라미터가 파일이면 서버 경로에 업로드 파일을 저장
			fileName = fileItem.getName(); // 전체경로 이름
			fileName = fileName.substring(fileName.lastIndexOf("\\") + 1); //경로는 자르고 파일 이름만 저장 
